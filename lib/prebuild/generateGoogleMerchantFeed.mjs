...
import { getProductAvailabilityConfig } from '../../data/commerce/productAvailability.mjs';
import { SITE_URL } from '../../data/site/constants.mjs';
import { productCatalog, productDefinitions } from '../products/catalog.mjs';

const pricingFile = path.join(process.cwd(), 'data', 'commerce', 'pricing.json');
...
async function buildItems() {
  ...
  for (const [productKey, product] of Object.entries(productDefinitions)) {
    ...
    for (const sku of product.variantSkus) {
      ...
      const title = merchantProduct.merchant.title(amount);
      const description = merchantProduct.merchant.description(amount);
