...
import { getProductAvailabilityConfig } from '../commerce/productAvailability.mjs';
import { formatBusinessDateTime } from '../dates/businessDate.mjs';
import { getScheduledSkuDiscount } from '../pricing/skuDiscounts.mjs';
...
function getVariantForFeed(product, sku) {
  const metadata = product.variantMetadata?.[sku] ?? {};
  return {
    sku,
    amount: Number(metadata.amount ?? String(sku).split('-').pop()) || 0,
...
async function buildItems() {
  ...
  for (const [productKey, product] of Object.entries(productDefinitions)) {
    const merchantProduct = productCatalog[productKey];
    const unavailableSkus = new Set(
      getProductAvailabilityConfig(productKey).unavailableSkus
    );
    ...
    const imageUrls = [];
    for (const image of merchantProduct.images) {
      imageUrls.push(await ensureMerchantImage(image.url));
    }

    for (const sku of product.variantSkus) {
      const variant = getVariantForFeed(product, sku);
      if (variant.hideFromMerchantFeed) {
        continue;
      }
      ...
      const scheduledDiscount = getScheduledSkuDiscount({ sku, basePrice });
      const saleEffectiveDate = scheduledDiscount?.validFrom
        ? `${formatBusinessDateTime(scheduledDiscount.validFrom)}/${formatBusinessDateTime(scheduledDiscount.validUntil, '23:59:59')}`
        : '';
      const salePriceBlock = scheduledDiscount
        ? [
            `      <g:sale_price>${escapeXml(formatPrice(scheduledDiscount.discountedPrice))}</g:sale_price>`,
            saleEffectiveDate
      ...
      const title = merchantProduct.merchant.title(variant.amount, variant);
      const description = merchantProduct.merchant.description(variant.amount, variant);
      const link = `${SITE_URL}${merchantProduct.canonicalUrl}`;
...
${additionalImageBlock}      <g:availability>${escapeXml(unavailableSkus.has(sku) ? 'out_of_stock' : merchantProduct.merchant.availability)}</g:availability>
      <g:price>${escapeXml(formatPrice(basePrice))}</g:price>
${salePriceBlock}      <g:condition>${escapeXml(merchantProduct.merchant.condition)}</g:condition>
      <g:brand>${escapeXml(merchantProduct.merchant.brand)}</g:brand>
      <g:identifier_exists>${escapeXml(merchantProduct.merchant.identifierExists)}</g:identifier_exists>
      <g:product_type>${escapeXml(merchantProduct.merchant.productType)}</g:product_type>
      <g:shipping>
        <g:country>FI</g:country>
        <g:service>${escapeXml(merchantProduct.merchant.shippingService)}</g:service>
