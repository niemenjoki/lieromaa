...
import { isWormHuntDiscountId } from '../wormHunt/config.mjs';
...
function isExpiredDiscount(localDiscount, today = getDateOnly()) {
  return (
    typeof localDiscount?.endsOn === 'string' &&
    localDiscount.endsOn.trim() &&
    today > localDiscount.endsOn
  );
}
...
export function createDiscountSourceData(localData, previousSourceData = {}) {
  ...
  const sourceDiscounts = getLocalDiscounts(localData)
    .filter((discount) => !isWormHuntDiscountId(discount?.id))
    .filter((discount) => !isExpiredDiscount(discount))
    .map((discount, index) => createSourceDiscount(discount, index, seenIds));
