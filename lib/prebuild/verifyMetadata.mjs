...
function validateHowTo(howTo, slug) {
  if (!howTo) {
    return;
  }

  if (!howTo.name) fail(`[${slug}] howTo is missing name`);
  if (!howTo.description) fail(`[${slug}] howTo is missing description`);
  if (!Array.isArray(howTo.steps) || howTo.steps.length === 0) {
    fail(`[${slug}] howTo must include at least one step`);
    return;
  }

  howTo.steps.forEach((step, index) => {
    if (!step.name) fail(`[${slug}] howTo.steps[${index}] is missing name`);
    if (!step.text) fail(`[${slug}] howTo.steps[${index}] is missing text`);
  });
}
...
for (const guide of guides) {
  ...
  // Lengths
  validateLength(seoTitle ?? title, 50, 65, 'seoTitle/title', slug);
  validateLength(description, 110, 160, 'description', slug);
}
