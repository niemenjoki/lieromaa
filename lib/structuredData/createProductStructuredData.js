...
function createOffer(product, variant, shippingRate) {
  ...
  const variantName =
    variant.salesUnit === 'weight' && variant.weightGrams
...
function createBreadcrumbNode(product) {
  return {
    '@type': 'BreadcrumbList',
    '@id': product.breadcrumbId,
    itemListElement: product.breadcrumbItems.map((item, index) => ({
      '@type': 'ListItem',
      position: index + 1,
      name: item.name,
      item: item.url,
...
export function createProductStructuredData(productKey) {
  ...
  const webPageNode = {
    ...
    breadcrumb: { '@id': product.breadcrumbId },
    inLanguage: 'fi',
  };
  ...
  const graph = [webPageNode, productNode, createBreadcrumbNode(product)];
