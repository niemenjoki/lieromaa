...
const GUIDE_CATEGORY_DESCRIPTIONS = Object.freeze({
  'kompostin hyödyntäminen': 'Matokakan kerääminen, viimeistely ja käyttö kasveille.',
  'kompostorin perustaminen':
    'Kompostorin valinta, käyttöönotto ja madot ensimmäisiin viikkoihin.',
  'kompostorin hoito': 'Ruokinta, olosuhteet ja tavallisimpien ongelmien ratkaisut.',
});
  ...
  return uniqueCategoryNames.map((categoryName) => {
    ...
    return {
      ...
      description:
        ...
        'Käytännön oppaita matokompostointiin.',
      count: categoryGuides.length,
      items: categoryGuides.slice(0, 2).map((guide) => ({
        href: `/opas/${categorySlug}/${guide.slug}`,
        label: guide.title,
      })),
    };
  });
