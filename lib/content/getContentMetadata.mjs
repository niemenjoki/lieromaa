...
export function getContentMetadata({ type, slug }) {
  ...
  const metadataPath = path.join(contentDir, 'meta.json');

  if (!fs.existsSync(metadataPath)) {
    throw new Error(`No meta.json found for content: ${slug}`);
  }
  ...
  try {
    ...
    throw new Error(`Failed to parse meta.json for content: ${slug} - ${err.message}`);
  }
