import { ORDER_FORM_MIN_FILL_MS } from '@/lib/copy/orderMessages';
...
import { getCartErrorMessage, getCartOrderQuote } from '@/lib/orders/cartOrder';
import { getOrderQuote } from '@/lib/orders/getOrderQuote';
import {
  findProductKeyBySku,
  getLegacyProductVariantBySku,
...
const FINNISH_POSTCODE_REGEX = /^\d{5}$/;
...
export function getOrderValidationMessage(error, language) {
  const messages = getTransactionMessages(language).orderValidation;
  return messages[error?.code] ?? messages.validation;
}
...
function sanitizeOptionalIsoDate(source, key) {
  const value = normalizeSingleLine(getField(source, key));
...
function parseCartItemsJson(value) {
  try {
  ...
  throw validationError('Cart items payload was invalid.', {
    code: 'cart_invalid',
    publicMessage: 'Ostoskoria ei voitu lukea. Päivitä sivu ja yritä uudelleen.',
  });
}

function createCartProductPayload(line) {
  return {
    key: line.productKey,
    name: line.name,
    sku: line.sku,
    ...
    salesUnit: line.salesUnit ?? null,
    weightGrams: line.weightGrams ?? null,
    estimatedWormCount: line.estimatedWormCount ?? null,
    unitPrice: line.unitPrice,
    itemTotal: line.itemTotal,
  };
}

function normalizePublicCartOrderSubmission(
  source,
  { now, sourceRequestId, providedSourceRequestId, language, country }
) {
  const rawCartItemsJson = sanitizeField(source, 'cart_items_json', {
    required: true,
    maxLength: 4000,
  });
  ...
  const cartItems = parseCartItemsJson(rawCartItemsJson);
  const plainDiscountCode = normalizeDiscountCode(
    sanitizeField(source, 'alennuskoodi', { maxLength: 80 })
  );
  const discount = plainDiscountCode
    ? resolveDiscountCode({ code: plainDiscountCode, now })
    : null;
  let quote;
  ...
  try {
    ...
    const publicMessage = getCartErrorMessage(error, language, 'cart_validation_failed');
    throw validationError(
      error instanceof Error ? error.message : 'Cart quote resolution failed.',
      {
        code: error?.code ?? 'cart_invalid',
        publicMessage,
      }
    );
  }
  ...
  if (!quote.items.length) {
    throw validationError('Cart was empty.', {
      code: 'cart_empty',
      publicMessage: 'Ostoskori on tyhjä.',
    });
  }

  if (plainDiscountCode && !discount) {
    throw validationError('Discount code was invalid or expired.', {
      code: 'invalid_discount_code',
      publicMessage: 'Alennuskoodi ei ole voimassa.',
    });
  }

  if (discount && quote.discountAmounts.totalAmount <= 0) {
    throw validationError('Discount code did not apply to the cart.', {
      code: 'discount_not_applicable',
      publicMessage: 'Alennuskoodi ei koske ostoskorin tuotteita.',
    });
  }

  const shippingOption = quote.shippingOption;
  ...
  const paymentAcknowledgement = sanitizeField(source, 'maksu_vahvistettu', {
    required: true,
    maxLength: 20,
  });
  ...
  if (
    (deliveryUsesPickupPoint || deliveryUsesHomeAddress) &&
    !FINNISH_POSTCODE_REGEX.test(postalCode)
  ) {
    throw validationError('Postal code was not a five-digit Finnish postcode.', {
      code: 'invalid_postcode',
      publicMessage: 'Anna suomalainen 5-numeroinen postinumero.',
    });
  }
  ...
  const renderedEarliestShippingDate = sanitizeOptionalIsoDate(
    source,
    'availability_earliest_shipping_date'
  );

  if (paymentAcknowledgement !== 'ymmarretty') {
    throw validationError('Cart payment acknowledgement is missing.', {
      code: 'payment_acknowledgement_required',
      publicMessage: 'Vahvista maksutapa ennen tilauksen lähettämistä.',
    });
  }
  ...
  const items = quote.items.map(createCartProductPayload);
  ...
  return {
    ...
    pricing: {
      currency: 'EUR',
      itemPrice: quote.itemSubtotal,
      shippingPrice: quote.shippingPrice,
      shippingLabel: shippingOption.label,
      extraCharges: [],
      frostProtectionSelected: false,
      frostProtectionPrice: 0,
      discount: discount
        ? {
            codePlain: plainDiscountCode,
            ...
            extraChargeAmount: quote.discountAmounts.extraChargeAmount,
    ...
    availability: {
      earliestShippingDate: renderedEarliestShippingDate,
    },
    notes: {
      customerMessage,
    },
    rawFields: {
      cart_items_json: rawCartItemsJson,
      maksu_vahvistettu: paymentAcknowledgement,
    },
  };
}
...
export function normalizePublicOrderSubmission(
  ...
  let language;
  try {
    language = normalizeLanguage(getField(source, 'language'));
  } catch {
    throw validationError('Language was invalid.', {
      code: 'invalid_language',
      publicMessage: 'Kieli ei ole kelvollinen.',
    });
  }
  const country = normalizeSingleLine(getField(source, 'country')).toUpperCase() || 'FI';
  if (country !== 'FI') {
    throw validationError('Country was not FI.', {
      code: 'invalid_country',
      publicMessage: 'Toimitusmaa voi olla vain Suomi.',
    });
  }
  ...
  if (sanitizeField(source, 'cart_items_json', { maxLength: 4000 })) {
    return normalizePublicCartOrderSubmission(source, {
      now,
      sourceRequestId,
      providedSourceRequestId,
      language,
  ...
  const submittedSalesUnit = sanitizeField(source, 'myyntiyksikko', { maxLength: 40 });
  const resolvedProductKey = findProductKeyBySku(sku, { includeLegacy: true });
  const productKey =
    sanitizeField(source, 'tuote_avain', { maxLength: 80 }) || resolvedProductKey;
  const submittedLegacySalesUnit = submittedSalesUnit || 'worm_count';
  const preferredLegacyVariant =
    submittedSalesUnit === 'weight'
      ? null
      : getLegacyProductVariantBySku(sku, {
          salesUnit: submittedLegacySalesUnit,
        });
  const currentVariant = getProductVariantBySku(sku);
  const fallbackLegacyVariant = getLegacyProductVariantBySku(sku, {
    salesUnit: submittedSalesUnit || null,
  });
  const variant = preferredLegacyVariant ?? currentVariant ?? fallbackLegacyVariant;
  ...
  if (!variant.isAvailable) {
    throw validationError(`SKU "${sku}" is currently unavailable.`, {
  ...
  const shippingOption = getShippingOption(productKey, shippingMethod, language);
  if (!shippingOption) {
  ...
  const selectedExtraCharges = Object.fromEntries(
    getProductOrderConfig(productKey, language).extraCharges.map((charge) => [
      charge.fieldName,
  ...
  try {
    quote = getOrderQuote({
      productKey,
      sku: variant.sku,
      legacy: Boolean(variant.isLegacy),
  ...
  const appliedDiscount =
    discount && quote.discountAmounts.totalAmount > 0 ? discount : null;
  ...
  return {
    ...
    product: {
      ...
      name: product.name,
      sku: variant.sku,
      quantity: variant.estimatedWormCount ?? variant.amount,
      salesUnit: variant.salesUnit ?? null,
      weightGrams: variant.weightGrams ?? null,
      estimatedWormCount: variant.estimatedWormCount ?? variant.amount,
    ...
    pricing: {
      ...
      discount: appliedDiscount
        ? {
            ...
            codeMasked: maskDiscountCode(plainDiscountCode),
            obfuscatedCode: appliedDiscount.obfuscatedCode,
    ...
    rawFields: {
      ...
      myyntiyksikko: submittedSalesUnit,
      paino_grammoina: sanitizeField(source, 'paino_grammoina', {
        maxLength: 40,
      }),
      arvioitu_matomäärä: sanitizeField(source, 'arvioitu_matomäärä', {
        maxLength: 40,
      }),
    },
  };
