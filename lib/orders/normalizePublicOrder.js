...
export function normalizePublicOrderSubmission(
  ...
  const phone = sanitizePhone(source, 'phone', {
    required: deliveryUsesPickupPoint || deliveryUsesHomeAddress,
  });
  const addressLine1 = sanitizeOptionalAddress(source, 'osoite', deliveryUsesHomeAddress);
  const postalCode = sanitizeOptionalAddress(
    source,
    'postinumero',
    deliveryUsesPickupPoint || deliveryUsesHomeAddress
  );
  const city = sanitizeOptionalAddress(source, 'toimipaikka', deliveryUsesHomeAddress);
  ...
  return {
    ...
    fulfillment: {
      ...
      address: deliveryUsesHomeAddress
        ? {
            line1: addressLine1,
            postalCode,
            city,
          }
        : deliveryUsesPickupPoint
          ? {
              line1: pickupPoint?.name || 'Noutopaikka valitaan postinumeron perusteella',
              postalCode: pickupPoint?.postalCode || postalCode,
              city: pickupPoint?.city || city,
            }
          : null,
      pickupPoint,
