...
function normalizePublicCartOrderSubmission(
  ...
  const paymentProvider = sanitizeField(source, 'payment_provider', {
    maxLength: 20,
  }).toUpperCase();
  ...
  if (!['INVOICE', 'STRIPE'].includes(paymentProvider)) {
    throw validationError('Cart payment provider is invalid.', {
      code: 'payment_provider_invalid',
      publicMessage: 'Valitse maksutapa ennen tilauksen lähettämistä.',
    });
  }
