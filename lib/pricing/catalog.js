import pricingData from '@/lib/commerce/pricingData';
import { getProductAvailabilityConfig } from '@/lib/commerce/productAvailability.mjs';
import { DEFAULT_LANGUAGE } from '@/lib/i18n/config.mjs';
import {
  formatCurrency as formatLocalizedCurrency,
  formatPrice as formatLocalizedPrice,
} from '@/lib/i18n/formatters.mjs';
import { getCommerceMessages } from '@/lib/i18n/messages.mjs';
import { getRoutePath } from '@/lib/i18n/routes.mjs';
import {
  cartAddOnsSource,
  cartShippingOptionsSource,
} from '@/lib/products/catalogSources.mjs';

import { productCatalog, productDefinitions } from '../products/catalog.mjs';
import { getActiveSkuDiscount } from './skuDiscounts.mjs';

const priceMap = pricingData?.prices ?? {};
const defaultOrderConfig = {
  defaultVariantAmount: null,
  variantSelectorPosition: 'beforeFulfillment',
  showWormAmountFinePrint: false,
  extraCharges: [],
  confirmationNote: '',
};

export function getProductPricing(productKey) {
  const product = productDefinitions[productKey];

  if (!product || typeof product !== 'object') {
    throw new Error(`Unknown product pricing key "${productKey}"`);
  }

  return product;
}

export function getProductAvailability(productKey) {
  const product = getProductPricing(productKey);
  const configuredAvailability = getProductAvailabilityConfig(productKey);
  const productSkus = new Set(product.variantSkus);
  const unavailableSkus = configuredAvailability.unavailableSkus.filter((sku) =>
    productSkus.has(sku)
  );

  return {
    earliestShippingDate: configuredAvailability.earliestShippingDate,
    unavailableSkus,
    hasLimitedAvailability: configuredAvailability.hasLimitedAvailability,
  };
}

function resolveLocalizedExtraCharge(charge, commerceMessages) {
  const copy = commerceMessages.extraCharges[charge.copyKey];
  if (!copy) {
    throw new Error(`Missing commerce copy for extra charge "${charge.copyKey}".`);
  }

  return {
    ...charge,
    ...copy,
  };
}

function resolveInvoiceTiming(order, commerceMessages) {
  return Object.fromEntries(
    Object.entries(order.invoiceTimingKeysByFulfillmentType ?? {}).map(
      ([fulfillmentType, messageKey]) => {
        const message = commerceMessages.order.invoiceTiming[messageKey];
        if (!message) {
          throw new Error(`Missing invoice timing message "${messageKey}".`);
        }

        return [fulfillmentType, message];
      }
    )
  );
}

export function getProductOrderConfig(productKey, language = DEFAULT_LANGUAGE) {
  const product = productCatalog[productKey];

  if (!product || typeof product !== 'object') {
    throw new Error(`Unknown product order key "${productKey}"`);
  }

  const commerceMessages = getCommerceMessages(language);
  const order = product.order ?? {};
  const orderCopy =
    commerceMessages.order[order.copyKey] ?? commerceMessages.order.default;
  const shippingHelperTexts = order.shippingHelperTextKey
    ? commerceMessages.shippingOptions[order.shippingHelperTextKey]?.helperTexts
    : [];

  if (!orderCopy) {
    throw new Error(`Missing commerce order copy for "${order.copyKey}".`);
  }

  return {
    ...defaultOrderConfig,
    ...order,
    ...orderCopy,
    variantDescriptionLinkHref: order.variantDescriptionLinkPageKey
      ? getRoutePath(order.variantDescriptionLinkPageKey, language)
      : undefined,
    getVariantLabel({ amount, priceFormatted, variant }) {
      if (typeof orderCopy.variantLabel === 'function') {
        return orderCopy.variantLabel({
          amount,
          weight: variant?.weightGrams ?? amount,
          estimatedWormCount: variant?.estimatedWormCount,
          priceFormatted,
          variant,
        });
      }

      return `${amount} - ${priceFormatted} €`;
    },
    submitButtonLabel(args = {}) {
      return typeof orderCopy.submitButtonLabel === 'function'
        ? orderCopy.submitButtonLabel(args)
        : orderCopy.submitButtonLabel;
    },
    shippingHelperTexts: shippingHelperTexts ?? [],
    invoiceTimingByFulfillmentType: resolveInvoiceTiming(order, commerceMessages),
    extraCharges: (order.extraCharges ?? []).map((charge) =>
      resolveLocalizedExtraCharge(charge, commerceMessages)
    ),
  };
}

export function getPriceForSku(sku) {
  return getSkuPriceDetails(sku).price;
}

function createCartAddOn(addOn, language = DEFAULT_LANGUAGE) {
  const basePrice = getBasePriceForSku(addOn.priceSku ?? addOn.sku);
  const copy = getCommerceMessages(language).addOns[addOn.copyKey];
  if (!copy) {
    throw new Error(`Missing commerce copy for add-on "${addOn.copyKey}".`);
  }

  return {
    ...addOn,
    ...copy,
    image: addOn.image
      ? {
          ...addOn.image,
          alt: copy[addOn.image.altKey] ?? '',
        }
      : null,
    basePrice,
    price: basePrice,
  };
}

export function getProductCartAddOns(productKey, language = DEFAULT_LANGUAGE) {
  return Object.values(cartAddOnsSource ?? {})
    .filter((addOn) => addOn.parentProductKey === productKey)
    .map((addOn) => createCartAddOn(addOn, language));
}

export function getCartAddOnBySku(sku, language = DEFAULT_LANGUAGE) {
  const normalizedSku = String(sku || '').trim();
  const addOn = cartAddOnsSource?.[normalizedSku];

  return addOn ? createCartAddOn(addOn, language) : null;
}

export function getBasePriceForSku(sku) {
  return Number(priceMap[sku]) || 0;
}

export function getSkuPriceDetails(
  sku,
  { basePrice: basePriceOverride, applyDiscount = true } = {}
) {
  const basePrice =
    basePriceOverride === undefined
      ? getBasePriceForSku(sku)
      : Number(basePriceOverride) || 0;
  const discount = applyDiscount ? getActiveSkuDiscount({ sku, basePrice }) : null;

  return {
    basePrice,
    price: discount?.discountedPrice ?? basePrice,
    discount,
  };
}

function getVariantMetadata(product, sku) {
  const metadata = product.variantMetadata?.[sku];
  return metadata && typeof metadata === 'object' ? metadata : {};
}

function getLegacyVariantMetadata(product, sku, { salesUnit = null } = {}) {
  const variants = Array.isArray(product.legacyVariants) ? product.legacyVariants : [];
  const normalizedSalesUnit =
    typeof salesUnit === 'string' && salesUnit.trim() ? salesUnit.trim() : null;

  if (normalizedSalesUnit) {
    const salesUnitMatch = variants.find(
      (variant) => variant?.sku === sku && variant?.salesUnit === normalizedSalesUnit
    );

    if (salesUnitMatch) {
      return salesUnitMatch;
    }
  }

  return variants.find((variant) => variant?.sku === sku) ?? null;
}

function getVariantAvailabilityStatus({ isAvailable, earliestShippingDate }) {
  if (!isAvailable) {
    return 'unavailable';
  }

  return earliestShippingDate ? 'limited' : 'available';
}

export function getProductVariants(productKey, { includeUnavailable = true } = {}) {
  const product = getProductPricing(productKey);
  const productAvailability = getProductAvailability(productKey);
  const unavailableSkuSet = new Set(productAvailability.unavailableSkus);

  const variants = product.variantSkus.map((sku) => {
    const metadata = getVariantMetadata(product, sku);
    const isAvailable = !unavailableSkuSet.has(sku);
    const { basePrice, price, discount } = getSkuPriceDetails(sku, {
      basePrice: metadata.price,
    });
    const amount = Number(metadata.amount ?? sku.split('-').pop()) || 0;

    return {
      sku,
      amount,
      salesUnit: metadata.salesUnit ?? null,
      weightGrams:
        metadata.weightGrams === undefined ? null : Number(metadata.weightGrams) || 0,
      estimatedWormCount:
        metadata.estimatedWormCount === undefined
          ? null
          : Number(metadata.estimatedWormCount) || 0,
      itemCount:
        metadata.itemCount === undefined ? null : Number(metadata.itemCount) || 0,
      binCount: metadata.binCount === undefined ? null : Number(metadata.binCount) || 0,
      solidBinCount:
        metadata.solidBinCount === undefined ? null : Number(metadata.solidBinCount) || 0,
      drilledBinCount:
        metadata.drilledBinCount === undefined
          ? null
          : Number(metadata.drilledBinCount) || 0,
      lidCount: metadata.lidCount === undefined ? null : Number(metadata.lidCount) || 0,
      includedCoirLiters:
        metadata.includedCoirLiters === undefined
          ? null
          : Number(metadata.includedCoirLiters) || 0,
      expansionSku:
        typeof metadata.expansionSku === 'string' ? metadata.expansionSku : null,
      baseSku: typeof metadata.baseSku === 'string' ? metadata.baseSku : null,
      isExpansion: Boolean(metadata.isExpansion),
      hideFromVariantSelector: Boolean(metadata.hideFromVariantSelector),
      hideFromPublicOffers: Boolean(metadata.hideFromPublicOffers),
      hideFromMerchantFeed: Boolean(metadata.hideFromMerchantFeed),
      basePrice,
      price,
      discount,
      isAvailable,
      isLegacy: false,
      availabilityStatus: getVariantAvailabilityStatus({
        isAvailable,
        earliestShippingDate: productAvailability.earliestShippingDate,
      }),
      earliestShippingDate: isAvailable
        ? productAvailability.earliestShippingDate
        : false,
    };
  });

  return includeUnavailable
    ? variants
    : variants.filter((variant) => variant.isAvailable);
}

export function getAvailableProductVariants(productKey) {
  return getProductVariants(productKey, { includeUnavailable: false });
}

function createLegacyProductVariant(productKey, sku, options = {}) {
  const product = getProductPricing(productKey);
  const metadata = getLegacyVariantMetadata(product, sku, options);
  if (!metadata) {
    return null;
  }

  const amount = Number(metadata.amount ?? sku.split('-').pop()) || 0;
  const { basePrice, price, discount } = getSkuPriceDetails(sku, {
    basePrice: metadata.price,
    applyDiscount: false,
  });

  return {
    sku,
    amount,
    salesUnit: metadata.salesUnit ?? 'worm_count',
    weightGrams:
      metadata.weightGrams === undefined ? null : Number(metadata.weightGrams) || 0,
    estimatedWormCount:
      metadata.estimatedWormCount === undefined
        ? amount
        : Number(metadata.estimatedWormCount) || 0,
    itemCount: metadata.itemCount === undefined ? null : Number(metadata.itemCount) || 0,
    binCount: metadata.binCount === undefined ? null : Number(metadata.binCount) || 0,
    solidBinCount:
      metadata.solidBinCount === undefined ? null : Number(metadata.solidBinCount) || 0,
    drilledBinCount:
      metadata.drilledBinCount === undefined
        ? null
        : Number(metadata.drilledBinCount) || 0,
    lidCount: metadata.lidCount === undefined ? null : Number(metadata.lidCount) || 0,
    includedCoirLiters:
      metadata.includedCoirLiters === undefined
        ? null
        : Number(metadata.includedCoirLiters) || 0,
    expansionSku:
      typeof metadata.expansionSku === 'string' ? metadata.expansionSku : null,
    baseSku: typeof metadata.baseSku === 'string' ? metadata.baseSku : null,
    isExpansion: Boolean(metadata.isExpansion),
    hideFromVariantSelector: Boolean(metadata.hideFromVariantSelector),
    hideFromPublicOffers: Boolean(metadata.hideFromPublicOffers),
    hideFromMerchantFeed: Boolean(metadata.hideFromMerchantFeed),
    basePrice,
    price,
    discount,
    isAvailable: true,
    isLegacy: true,
    availabilityStatus: 'available',
    earliestShippingDate: false,
  };
}

export function getProductVariant(productKey, amount, options) {
  return (
    getProductVariants(productKey, options).find(
      (variant) => String(variant.amount) === String(amount)
    ) ?? null
  );
}

export function findProductKeyBySku(sku, { includeLegacy = false } = {}) {
  const plainSku = String(sku || '').trim();
  if (!plainSku) {
    return null;
  }

  return (
    Object.entries(productDefinitions).find(([, product]) => {
      if (product.variantSkus.includes(plainSku)) {
        return true;
      }

      return includeLegacy && Boolean(getLegacyVariantMetadata(product, plainSku));
    })?.[0] ?? null
  );
}

export function getProductVariantBySku(sku, { legacy = false, salesUnit = null } = {}) {
  const productKey = findProductKeyBySku(sku, { includeLegacy: legacy });
  if (!productKey) {
    return null;
  }

  if (legacy) {
    const legacyVariant = createLegacyProductVariant(productKey, sku, { salesUnit });
    if (legacyVariant) {
      return legacyVariant;
    }
  }

  return getProductVariants(productKey).find((variant) => variant.sku === sku) ?? null;
}

export function getLegacyProductVariantBySku(sku, options = {}) {
  const productKey = findProductKeyBySku(sku, { includeLegacy: true });
  return productKey ? createLegacyProductVariant(productKey, sku, options) : null;
}

export function isSkuAvailable(sku) {
  return Boolean(getProductVariantBySku(sku)?.isAvailable);
}

function resolveShippingOptionPrices(configuredOptions, language) {
  const shippingMessages = getCommerceMessages(language).shippingOptions;

  if (configuredOptions.length > 0) {
    return configuredOptions.map((option) => {
      const copy = shippingMessages[option.copyKey];
      if (!copy) {
        throw new Error(`Missing commerce copy for shipping option "${option.copyKey}".`);
      }

      return {
        ...option,
        ...copy,
        price:
          option.price !== undefined
            ? Number(option.price) || 0
            : option.priceSku
              ? getPriceForSku(option.priceSku)
              : 0,
      };
    });
  }

  return [];
}

export function getProductShippingOptions(productKey, language = DEFAULT_LANGUAGE) {
  const product = getProductPricing(productKey);
  const configuredOptions = Array.isArray(product.shippingOptions)
    ? product.shippingOptions
    : [];

  return resolveShippingOptionPrices(configuredOptions, language);
}

export function getCartShippingOptions(language = DEFAULT_LANGUAGE) {
  return resolveShippingOptionPrices(
    Array.isArray(cartShippingOptionsSource) ? cartShippingOptionsSource : [],
    language
  );
}

export function getCartShippingOption(optionId, language = DEFAULT_LANGUAGE) {
  return (
    getCartShippingOptions(language).find((option) => option.id === optionId) ?? null
  );
}

export function getShippingOption(productKey, optionId, language = DEFAULT_LANGUAGE) {
  return (
    getProductShippingOptions(productKey, language).find(
      (option) => option.id === optionId
    ) ?? null
  );
}

export function getDefaultShippingOption(productKey, language = DEFAULT_LANGUAGE) {
  return getProductShippingOptions(productKey, language)[0] ?? null;
}

export function getProductOfferStats(productKey, { includeUnavailable = false } = {}) {
  const prices = getProductVariants(productKey, { includeUnavailable })
    .map((variant) => Number(variant.price) || 0)
    .sort((a, b) => a - b);

  return {
    lowPrice: prices[0] ?? 0,
    highPrice: prices[prices.length - 1] ?? 0,
    offerCount: prices.length,
  };
}

export function getAllSkus() {
  return Object.keys(priceMap).filter((sku) => typeof sku === 'string' && sku.length > 0);
}

export function formatPrice(value, language = DEFAULT_LANGUAGE) {
  return formatLocalizedPrice(value, language);
}

export function formatCurrency(value, language = DEFAULT_LANGUAGE) {
  return formatLocalizedCurrency(value, language);
}

export function formatSchemaPrice(value) {
  return (Number(value) || 0).toFixed(2);
}
