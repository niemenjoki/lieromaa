...
function getLegacyVariantMetadata(product, sku) {
  return (
    (Array.isArray(product.legacyVariants) ? product.legacyVariants : []).find(
...
function createLegacyProductVariant(productKey, sku) {
  const product = getProductPricing(productKey);
  const metadata = getLegacyVariantMetadata(product, sku);
  if (!metadata) {
    return null;
  }
...
export function getProductVariantBySku(sku, { legacy = false } = {}) {
  ...
  if (legacy) {
    const legacyVariant = createLegacyProductVariant(productKey, sku);
    if (legacyVariant) {
...
export function getLegacyProductVariantBySku(sku) {
  const productKey = findProductKeyBySku(sku, { includeLegacy: true });
  return productKey ? createLegacyProductVariant(productKey, sku) : null;
}
