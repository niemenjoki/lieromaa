...
async function testPostMetadata() {
  ...
  for (const file of files) {
    ...
    if (hasH1) {
      issues.push(`${colors.red}[${file}] duplicate H1 headings${colors.reset}\n`);
    }
  ...
  if (issues.length) {
    console.log(`${colors.yellow}${colors.bold}⚠️  Metadata issues found:${colors.reset}\n`);
    for (const issue of issues) console.log(' - ' + issue);
    console.log(`\n${colors.red}❌ Found ${issues.length} post metadata issues${colors.reset}`);
  } else {
    console.log(`${colors.green}✅ All post metadata checks passed.${colors.reset}`);
  }
}
